...
export function BookingComLogo({ className, iconOnly, monochromeHover }: LogoProps) {
  const mark = (
    <div
      className={cn(
        "relative flex size-8 shrink-0 items-center justify-center rounded-[22%] bg-[#003580] shadow-[0_2px_8px_-2px_rgba(0,0,0,0.4)] transition-all duration-300",
...
export function HostelworldLogo({ className, iconOnly, monochromeHover }: LogoProps) {
  const mark = (
    <div
      className={cn(
        "flex size-8 shrink-0 items-center justify-center rounded-[22%] bg-[#F25621] shadow-[0_2px_8px_-2px_rgba(0,0,0,0.4)] transition-all duration-300",
        monochromeHover && "saturate-75 opacity-80 group-hover:saturate-100 group-hover:opacity-100",
        iconOnly && className
      )}
    >
