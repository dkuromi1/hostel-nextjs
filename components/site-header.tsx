        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {navLinks.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="inline-block rounded-full px-4 py-2 text-sm font-medium text-slate-600 transition-all duration-300 hover:scale-[1.02] active:scale-95 hover:bg-white hover:text-slate-950"
            >
              {item.label}
            </Link>
          ))}
